...
#+BEGIN_SRC elisp
  ...
  (defface mode-line-file-name
      ...
      "A face for a file name in the mode line.")

  (defface mode-line-star-buffer
      '((t . (:weight bold :foreground "white" :background "gray")))
      "A face for the name of buffers of the shape '*name*' in the mode line.")
...
#+BEGIN_SRC emacs-lisp 
 '(default ((t (:foreground "<<std-color>>" :background "<<std-background>>" :distant-foreground "light gray"))))
...
** Ongoing Typing
The cursor font is not specified so it can be specified in the configuration itself (and in particular match the beacon).

#+BEGIN_SRC emacs-lisp 
 '(hl-line ((t (:background "<<highlight-background>>" :foreground "<<highlight-color>>"))))
...
** Outline-mode
Dark brown: 79492d
#+NAME: outline-heading-template
#+BEGIN_SRC python :var space=0 :var color="black" :var background_hl=0 :var height=1.0 :var weight="normal" :noweb yes :results output :tangle no
  print(
      "((t (:background \"{}\" :foreground \"{}\" :height {} :weight {} :box (:line-width ({} . {}) :color \"{}\"))))".format(
          "<<highlight-color>>" if background_hl else "<<std-background>>",
          "<<highlight-background>>" if background_hl else color,
          height,
          weight,
          space,
          space,
...
#+BEGIN_SRC emacs-lisp  
'(outline-1 <<outline-heading-template(height=1.7, weight="extra-bold", space=10, background_hl=1)>>)
'(outline-2 <<outline-heading-template(color="#222222", height=1.5, weight="bold", space=10)>>)
'(outline-3 <<outline-heading-template(color="#444444", height=1.2, weight="semi-bold", space=6)>>)
'(outline-4 <<outline-heading-template(color="#555555", height=1.05, weight="semi-bold", space=3)>>)
'(outline-5 <<outline-heading-template(color="#666666", height=1.0, weight="semi-bold", space=2)>>)
#+END_SRC 


#+BEGIN_SRC emacs-lisp :tangle no
'(outline-1 <<outline-heading-template(color="#3581b6", height=1.8, weight="extra-bold", space=14)>>)
'(outline-2 <<outline-heading-template(color="#2a6995", height=1.5, weight="bold", space=10)>>)
'(outline-3 <<outline-heading-template(color="#315e7d", height=1.2, weight="semi-bold", space=6)>>)
'(outline-4 <<outline-heading-template(color="#2f5169", height=1.05, weight="semi-bold", space=3)>>)
'(outline-5 <<outline-heading-template(color="#2b404f", height=1.0, weight="semi-bold", space=2)>>)
#+END_SRC 
...
#+BEGIN_SRC emacs-lisp 
'(org-document-title ((t (:background "<<highlight-color>>" :foreground "<<highlight-background>>" :height 2.0 :weight semi-bold))))
'(org-document-info-keyword  ((t (:background "<<structure-background>>" :foreground "<<structure-color>>" :height 0.8 :weight semi-light))))
#+END_SRC

*** Code blocks
#+BEGIN_SRC emacs-lisp 
  '(org-block-begin-line  ((t (:background "<<code-background>>" :foreground "<<code-color>>" :weight semi-light :height 0.8 :box (:line-width (1 . 7) :color "<<code-background>>")))))
  
  '(org-block ((t (:background "<<code-background>>" :foreground "#444444"))))
  '(org-block-end-line  ((t (:inherit 'org-block-begin-line))))
#+END_SRC

*** Org specific Concepts
#+BEGIN_SRC emacs-lisp 
  '(org-verbatim ((t (:background "<<code-background>>" :foreground "chartreuse4" :weight semi-light))))
  ...
  '(org-ellipsis  ((t (:background "<<structure-background>>" :foreground "<<structure-color>>" :height 0.7))))
#+END_SRC 

*** Headings
The headings are part of the regular content of the file, so they do not need a special color.
...
*** Keywords (TODO et al.) :attentionNeeded:
...
** Adding missing fonts in python/cython :stable:
I use =python= and =cython= a lot, and I have found two problems. For =python=, class names are not highlighted in a specific way. However, a class name is a meaningful information in terms of structure, so I highlight it.

Not having a dedicated cython mode, I instead use =python-mode= and highlight some of the corresponding keywords by hand.
